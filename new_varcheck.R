...
# Look at these hospitals
hospdata_analysis %>%
  filter(mcaid_prop_discharges == 1) %>%
  select(mcrnum, state, year, name, mcaid_discharges, tot_discharges, beds) %>%
  arrange(mcrnum, year)

# Are these small hospitals?
hospdata_analysis %>%
  filter(mcaid_prop_discharges == 1) %>%
  summarise(
    n_obs = n(),
    ...
    avg_discharges = mean(tot_discharges, na.rm = TRUE),
    median_discharges = median(tot_discharges, na.rm = TRUE)
  )

# Check if it's consistent across years for these hospitals
hospdata_analysis %>%
  filter(mcaid_prop_discharges == 1) %>%
  count(mcrnum, sort = TRUE)


# Check their typectrl - did they slip through your filter?
hospdata_analysis %>%
  filter(mcrnum %in% c("54122", "54133", "264010", "330410", "332022", "454014")) %>%
  distinct(mcrnum, name, typectrl) %>%
  arrange(mcrnum)

# How many hospitals have missing typectrl?
hospdata_analysis %>%
  filter(is.na(typectrl)) %>%
  summarise(
...
hospdata_analysis %>%
  filter(is.na(typectrl)) %>%
  distinct(mcrnum, name, state) %>%
  head(20)


# Sample of hospitals with missing typectrl
hospdata_analysis %>%
  filter(is.na(typectrl)) %>%
  distinct(mcrnum, name, state, beds) %>%
  arrange(desc(beds)) %>%
  head(30)

# Are they consistently missing typectrl, or just some years?
hospdata_analysis %>%
  group_by(mcrnum) %>%
  summarise(
    n_years = n(),
    n_missing_typectrl = sum(is.na(typectrl)),
    pct_missing = n_missing_typectrl / n_years * 100,
    name = first(name)
  ) %>%
  filter(n_missing_typectrl > 0) %>%
  arrange(desc(n_years)) %>%
  head(20)

# Check if typectrl varies within hospitals
hospdata_analysis %>%
  group_by(mcrnum) %>%
  summarise(
    n_typectrl_values = n_distinct(typectrl, na.rm = TRUE),
    typectrl_values = paste(unique(typectrl), collapse = ", "),
    has_NA = any(is.na(typectrl))
  ) %>%
  filter(has_NA) %>%
  head(50)
