# Meta --------------------------------------------------------------------
## Author:        Jill Wilkins
## Date Created:  10/9/2025
## Date Edited:   11/14/2025
## Goal:          og Summary Statistics Table         
## 

library(knitr)
library(kableExtra)

# set up  to make tables
# key variables
sum_vars <- c("private_prop_discharges", "mcaid_prop_discharges", "ucc_prop", "psyemhos", "alchhos", 
                  "cost_to_charge", "rural")

var_names <- c(
  private_prop_discharges = "Non Public Proportion of Discharges",
  mcaid_prop_discharges = "Medicaid Proportion of Discharges",
  mcaid_prop = "Medicaid Proportion",
  ucc_prop = "Uncompensated Care Proportion",
  cost_per_discharge = "Cost per Discharge",
  cost_to_charge = "Cost to Charge Ratio"
)

# recall hospdata is only for SERV == 10, and so Years > 2008 basically? 
# remove outliers
hospdata_clean <- hospdata %>%
  filter(
    !is.na(mcaid_prop_discharges),
    year < 2020,
    mcaid_prop_discharges >= quantile(mcaid_prop_discharges, 0.01, na.rm = TRUE),
    mcaid_prop_discharges <= quantile(mcaid_prop_discharges, 0.99, na.rm = TRUE), 
    private_prop_discharges >= quantile(private_prop_discharges, 0.01, na.rm = TRUE),
    private_prop_discharges <= quantile(private_prop_discharges, 0.99, na.rm = TRUE), 
    ucc_prop >= quantile(ucc_prop, 0.01, na.rm = TRUE),
    ucc_prop <= quantile(ucc_prop, 0.99, na.rm = TRUE)
  )

# TABLE: SUMMARY STATISTICS (full, with tax, no tax) 
# FULL SAMPLE: hospital level means
hospmeans <- hospdata_clean %>%
   filter(year > 2004, year < 2025) %>%
   group_by(mcrnum) %>%
   summarise(across(all_of(names(var_names)),  # variables to summarise
                   ~mean(.x, na.rm = TRUE),
                   .names = "{.col}")) %>%
   ungroup()

hosp_sum <- hospmeans %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(), names_to = c("variable", "stat"), names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(
    mean = as.numeric(mean),
    sd = as.numeric(sd),
    stat_string = sprintf("%.3f (%.3f)", mean, sd), 
    variable = ifelse(variable %in% names(var_names), var_names[variable], variable)
  ) %>%
  select(variable, stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      stat_string = as.character(nrow(hospmeans))
    )
  )

kable(hosp_sum, format = "latex", booktabs = TRUE,
      caption = "Summary Statistics by Treatment Status",
      col.names = c("Dependent Variable", "Mean (SD)"),
      align = "lc")

# Observation with Tax: Group Means 
tax_sum <- hospdata_clean %>%
  filter(yes_tax == 1) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Treated = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Treated = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 1])))
  )

View(tax_sum)

# Observation without Tax: Group Means
notax_sum <- hospdata_clean %>%
  filter(yes_tax == 0) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Control = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Control = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 0]))
    )
  )
View(notax_sum)
# Combine with hosp_sum
all_sum <- hosp_sum %>%
    left_join(tax_sum, by = "variable") %>%
    left_join(notax_sum, by = "variable")

kable(all_sum, format = "latex", booktabs = TRUE,
      caption = "Summary Statistics by Presence of Provider Tax",
      col.names = c("Dependent Variable", "All Hospitals", "With Tax", "Without Tax"),
      align = "lccc")

# TABLE: SUMMARY STATISTICS (by treatment status)

hospdata_clean %>%
  filter(year > 2004, treated == 1) %>%
  summarise(across(all_of(names(var_names)), ~sum(!is.na(.x))))
setdiff(names(var_names), names(hospdata_clean))

# treated before 2020
treated_hos2020 <- hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, year < 2020) %>%
  group_by(mcrnum) %>%  # group by hospital
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE)),
                   .names = "{.col}")) %>%
  ungroup() %>%
  summarise(across(everything(),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.2f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, est = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Number of Hospitals",
      est = as.character(n_distinct(hospdata$mcrnum[hospdata$treatment_group == "treated" & hospdata$yes_tax == 1]))
    )
  )
View(treated_hos2020)

# always hospitals only 
always_hos <- hospdata %>%
  filter(always == 1, year < 2020) %>%
  group_by(mcrnum) %>%  # group by hospital
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE)),
                   .names = "{.col}")) %>%
  ungroup() %>%
  summarise(across(everything(),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.2f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, est = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Number of Hospitals",
      est = as.character(n_distinct(hospdata$mcrnum[hospdata$always == 1]))
    )
  )

# never hospitals only
never_hos <- hospdata %>%
  filter(never == 1) %>%
  group_by(mcrnum) %>%  # group by hospital
  summarise(across(names(var_names),    
                   list(mean = ~mean(.x, na.rm = TRUE)),
                   .names = "{.col}")) %>%
  ungroup() %>%
  summarise(across(everything(),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%      
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.2f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, est = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Number of Hospitals",
      est = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$never == 1]))
    )
  )
unique(hospdata_clean$treatment_group)
# not treated by 2020 
notby2020 <- hospdata_clean %>%
  filter(treatment_group == "not yet by 2020", year < 2020) %>%
  group_by(mcrnum) %>%  # group by hospital
  summarise(across(names(var_names),    
                   list(mean = ~mean(.x, na.rm = TRUE)),
                   .names = "{.col}")) %>%
  ungroup() %>%
  summarise(across(everything(),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%      
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.2f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, est = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Number of Hospitals",
      est = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$treatment_group == "not yet by 2020"]))
    )
  )
View(notby2020)

# Combine all 
all_groups <- treated_hos %>%
    left_join(treated_hos2020, by = "variable") %>%
    left_join(always_hos, by = "variable") %>%
    left_join(never_hos, by = "variable") 
View(all_groups)

kable(all_groups, format = "latex", booktabs = TRUE,
      caption = "Summary Statistics by Treatment Status",
      col.names = c("Dependent Variable", "Treated Hospitals", "Treated by 2020", "Always Have Tax", "Never Have Tax"),
      align = "lcccc")
unique(hospdata_clean$state[hospdata_clean$notyet2020 == 1])
unique(hospdata$state[hospdata$notyet2020 == 1])

rm(table3)
# table for treated by 2020, not treated by 2020, and always 
table3 <- treated_hos2020 %>%
  left_join(notby2020, by = "variable") 
  # %>%
  # left_join(always_hos, by= "variable")
  View(table3)

kable(table3, format = "latex", booktabs = TRUE,
      caption = "Summary Statistics by Treatment Status",
      col.names = c("Dependent Variable", "Treated Hospitals", "Not Yet Treated by 2020"),
      align = "lcc")

# DENSITY PLOTS
ggplot(filter(hospdata_clean, treated == 1), aes(x = mcaid_prop_discharges)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
  labs(title = "Density of Medicaid Proportion of Discharges",
       x = "Medicaid Proportion of Discharges", y = "Density") +
  theme_minimal()



# now i want observations of eventually treated pre, post, and not yet treated by 2020 pre, post 
#########IGNORE
# Observation with Tax: Group Means 
pre_treated <- hospdata_clean %>%
  filter(yes_tax == 0 & treated_by_2020 == 1) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Treated = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Treated = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 0 & hospdata_clean$treated_by_2020 == 1])))
  )
View(hospdata_clean %>%
  filter(yes_tax == 0 & treated_by_2020 == 1))

pre_notyet <- hospdata_clean %>%
  filter(yes_tax == 0 & notyet2020 == 1) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Treated = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Treated = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 0 & hospdata_clean$notyet2020 == 1])))
  )

post_treated <- hospdata_clean %>%
  filter(yes_tax == 1 & treated_by_2020 == 1) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Treated = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Treated = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 1 & hospdata_clean$treated_by_2020 == 1])))
  )

post_notyet <- hospdata_clean %>%
  filter(yes_tax == 1 & notyet2020 == 1) %>%
  summarise(across(names(var_names),
                   list(mean = ~mean(.x, na.rm = TRUE),
                        sd = ~sd(.x, na.rm = TRUE)),
                   .names = "{.col}_{.fn}")) %>%
  pivot_longer(cols = everything(),
               names_to = c("variable", "stat"),
               names_pattern = "(.+)_([^_]+)") %>%
  pivot_wider(names_from = stat, values_from = value) %>%
  mutate(stat_string = sprintf("%.3f (%.3f)", mean, sd),
         variable = var_names[variable]) %>%
  select(variable, Treated = stat_string) %>%
  bind_rows(
    tibble(
      variable = "Total Hospitals",
      Treated = as.character(n_distinct(hospdata_clean$mcrnum[hospdata_clean$yes_tax == 1 & hospdata_clean$notyet2020 == 1])))
  )
rm(time_treat_sum)
time_treat_sum <- pre_treated %>%
    left_join(pre_notyet, by = "variable") %>%
    left_join(post_treated, by = "variable") %>%
    left_join(post_notyet, by = "variable")

kable(time_treat_sum, format = "latex", booktabs = TRUE,
      caption = "Summary Statistics by Pre/Post Treated/Not Yet Treated",
      col.names = c("Dependent Variable", "Pre Treatment Group", "Pre Not Yet Treated", "Post Treatment Group", "Post Not Yet Treated"),
      align = "lcccc")


summary(hospdata$cost_to_charge[hospdata$treatment_group == "treated" & hospdata$yes_tax == 1 & hospdata$year < 2020])
subset_data <- hospdata$cost_to_charge[
  hospdata$treatment_group == "treated" &
  hospdata$cost_to_charge >= quantile(hospdata$cost_to_charge, 0.01, na.rm = TRUE) &
  hospdata$cost_to_charge <= quantile(hospdata$cost_to_charge, 0.99, na.rm = TRUE) &
  hospdata$year < 2020
]
c(mean = mean(subset_data, na.rm = TRUE), sd = sd(subset_data, na.rm = TRUE))


# mean of alchhos among treated and not yet treated 
hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(alchbd), year < 2020) %>%
  summarise(mean_alchbd = mean(alchbd, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(alchbd), year < 2020) %>%
  summarise(mean_alchbd = mean(alchbd, na.rm = TRUE))

# standard deviations 
hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(alchhos), year < 2020) %>%
  summarise(sd_alchhos = sd(alchhos, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(alchhos), year < 2020) %>%
  summarise(sd_alchhos = sd(alchhos, na.rm = TRUE))

library(dplyr)

# Collapse to hospital-level averages first
hosp_level <- hospdata %>%
  filter(year < 2020, !is.na(alchbd), !is.na(alchhos)) %>%
  group_by(mcrnum, treatment_group, yes_tax) %>%
  summarise(
    mean_alchbd = mean(alchbd, na.rm = TRUE),
    mean_alchhos = mean(alchhos, na.rm = TRUE),
    sd_alchbd = sd(alchbd, na.rm = TRUE),
    sd_alchhos = sd(alchhos, na.rm = TRUE),
    .groups = "drop"
  )

# Summarize by treatment group
summary_stats <- hosp_level %>%
  filter(treatment_group == "treated" & yes_tax| treatment_group == "not yet by 2020") %>%
  group_by(treatment_group) %>%
  summarise(
    mean_alchbd = mean(mean_alchbd, na.rm = TRUE),
    sd_alchbd = sd(sd_alchbd, na.rm = TRUE),
    mean_alchhos = mean(mean_alchhos, na.rm = TRUE),
    sd_alchhos = sd(sd_alchhos, na.rm = TRUE),
    n_hospitals = n()
  )

summary_stats


hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(psybd), year < 2020) %>%
  summarise(mean_psyembd = mean(psybd, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(psybd), year < 2020) %>%
  summarise(mean_psybd = mean(psybd, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(obbd), year < 2020) %>%
  summarise(mean_obbd = mean(obbd_share, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(obbd), year < 2020) %>%
  summarise(mean_obbd = mean(obbd_share, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(beds), beds > 0, year < 2020) %>%
  summarise(mean_beds = mean(beds, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(beds), beds > 0, year < 2020) %>%
  summarise(mean_beds = mean(beds, na.rm = TRUE))

# means of rural among treated and not yet treated at hospital level 
hospdata %>%
  filter(treatment_group == "treated", yes_tax == 1, !is.na(rural), year < 2020) %>%
  distinct(mcrnum, rural) %>%
  summarise(mean_rural = mean(rural, na.rm = TRUE))

hospdata %>%
  filter(treatment_group == "not yet by 2020", !is.na(rural), year < 2020) %>%
  distinct(mcrnum, rural) %>%
  summarise(mean_rural = mean(rural, na.rm = TRUE))


# TRY AGAIN 
hospdata_clean %>%
  filter(
    treatment_group == "not treated by 2020" |
      (treatment_group == "treated" & year < firsttax), 
    !is.na(beds)
  ) %>%
  summarise(mean_y = mean(rural, na.rm = TRUE), 
            sd_y = sd(rural, na.rm = TRUE))

hospdata_clean %>%
  filter(
    treatment_group == "not treated by 2020" |
      (treatment_group == "treated" & year < firsttax), 
  ) %>%
  summarise(
    n = n(),
    mean_y = mean(beds, na.rm = TRUE), 
    sd_y = sd(beds, na.rm = TRUE)
  )

library(dplyr)

# This is what i want to automate after 11/5 
# CONTROL GROUP SUM 
hospdata_clean %>%
  # Step 0: filter for control/pre-treatment group
  filter(
    treatment_group == "not treated by 2020" |
      (treatment_group == "treated" & year < firsttax), 
    !is.na(beds), 
  ) %>%
  # Step 1: compute hospital-level mean
  group_by(mcrnum) %>%
  summarise(hosp_mean_y = mean(rural, na.rm = TRUE)) %>%
  ungroup() %>%
  # Step 2: compute mean and SD across hospitals
  summarise(
    mean_y = mean(hosp_mean_y, na.rm = TRUE),
    sd_y = sd(hosp_mean_y, na.rm = TRUE),
    n_hospitals = n()
  )

# TREATED GROUP SUM 
hospdata_clean %>%
  # Step 0: filter for control/pre-treatment group
  filter(
    treatment_group == "treated" & yes_tax == 1, 
    !is.na(beds)
  ) %>%
  # Step 1: compute hospital-level mean
  group_by(mcrnum) %>%
  summarise(hosp_mean_y = mean(private_prop_discharges, na.rm = TRUE)) %>%
  ungroup() %>%
  # Step 2: compute mean and SD across hospitals
  summarise(
    mean_y = mean(hosp_mean_y, na.rm = TRUE),
    sd_y = sd(hosp_mean_y, na.rm = TRUE),
    n_hospitals = n()
  )
